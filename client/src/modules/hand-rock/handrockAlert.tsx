...
interface RockPaperScissorsAlertProps {
  open: boolean;
...
const RockPaperScissorsAlert = ({
  open,
  setOpen,
  userChoice,
  results,
}: RockPaperScissorsAlertProps) => {
  ...
  useEffect(() => {
    ...
    const didUserWin = (userChoice: number, result: number) => {
        ...
        <AlertDialogHeader className={"w-full h-full"}>
          ...
          <AlertDialogDescription className="h-[55vh] overflow-y-scroll no-scrollbar p-4 border-2 border-black scroll-smooth">
            {skipAnimation ? (
              ...
              <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
                {visibleResults.map((result: number, index: number) => (
                  <div
                    key={index}
                    className="w-full aspect-square rounded-full flex items-center justify-center"
                  >
