...
export class BallManager {
  ...
  addBall(startX?: number) {
    const newBall = new Ball(
      ...
      this.sinks,
      (index: number) => {
        this.balls = this.balls.filter((ball: Ball) => ball !== newBall);
  ...
  triggerSinkColorChange(index: number) {
  ...
  drawObstacles() {
    ...
    this.obstacles.forEach((obstacle: Obstacle) => {
      if (obstacle.isChangingColor) {
        const elapsedTime = currentTime - (obstacle.colorChangeStartTime as number);
