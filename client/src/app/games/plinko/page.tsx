...
const Page = () => {
  ...
  const [ballManager, setBallManager] = useState<BallManager | undefined>();
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const dispath = useAppDispatch();
  const { ctx, ready } = useGameContext();
  const [, setResult] = useState<number>();

  const mappedPlinkoData: Record<string, number[]> = {
    "-8": simulationData[0],
    "-7": simulationData[1],
    "-6": simulationData[2],
    "-5": simulationData[3],
  ...
  const handlePlay = async () => {
    ...
    if (!sum) return;
    // console.log(sum);
    const arrayToBePicked = mappedPlinkoData[Number(sum)];
